...
// This specifies the GPIO that the SS/CS pin the SD reader is connected to.
const int SD_CHIP_SELECT = A2;
SdFat sd;
SdCardLogHandler *logHandler;
...
void setup()
{
	...
	logHandler = new SdCardLogHandler(sd, SD_CHIP_SELECT, SPI_FULL_SPEED);
...
void loop() {
	...
	case CONNECT_REPORT_STATE:
		{
			Log.info("connected to cellular network!");
			runCellularTests();

			{
				...
				Log.info("ping addr %s=%d", addrStr, ok);
			}
...
void runCellularTests() {
	...
	Log.info("rssi=%d, qual=%d", rssiQual.rssi, rssiQual.qual);
...
void printCellData(CellularHelperEnvironmentCellData *data) {
	...
	Log.info("%s %s %s %d bars", whichG, operatorName, data->getBandString().c_str(), data->getBars());
}
